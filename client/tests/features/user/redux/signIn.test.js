  ...
  it('dispatches success action when signIn succeeds', () => {
    ...
    return store.dispatch(signIn())
      .then(() => {
        ...
        expect(actions[0]).toHaveProperty('type', USER_SIGN_IN_BEGIN);
        expect(actions[1]).toHaveProperty('type', USER_SIGN_IN_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when signIn fails', () => {
    ...
    return store.dispatch(signIn({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).toHaveProperty('type', USER_SIGN_IN_BEGIN);
        expect(actions[1]).toHaveProperty('type', USER_SIGN_IN_FAILURE);
