...
import {
  ...
  doSignIn,
  reducer,
} from 'src/features/examples/redux/signIn';

describe('examples/redux/signIn', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('correct action by signIn', () => {
    expect(signIn()).to.have.property('type', EXAMPLES_SIGN_IN_BEGIN);
  });

  it('returns correct action by dismissSignInError', () => {
    expect(dismissSignInError()).to.have.property('type', EXAMPLES_SIGN_IN_DISMISS_ERROR);
  });
  ...
  it('dispatches EXAMPLES_SIGN_IN_SUCCESS action when succeeded', () => {
  ...
  it('dispatches EXAMPLES_SIGN_IN_FAILURE action when failed', () => {
  ...
  // reducer tests
  it('handles action type EXAMPLES_SIGN_IN_BEGIN correctly', () => {
  ...
  it('handles action type EXAMPLES_SIGN_IN_SUCCESS correctly', () => {
    const prevState = { signInPending: true };
    const state = reducer(
      prevState,
      { type: EXAMPLES_SIGN_IN_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.signInPending).to.be.false;
  });

  it('handles action type EXAMPLES_SIGN_IN_FAILURE correctly', () => {
    const prevState = { signInPending: true };
    const state = reducer(
      prevState,
      { type: EXAMPLES_SIGN_IN_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.signInPending).to.be.false;
  ...
  it('handles action type EXAMPLES_SIGN_IN_DISMISS_ERROR correctly', () => {
